...
class DualLoss(nn.Module):
    def __init__(self, ratio_decay=True, epochs=50):
        super(DualLoss, self).__init__()
        # self.criterion = DiceBCELoss().cuda()
        self.criterion = nn.BCELoss().cuda()
        self.ratio_decay = ratio_decay
        self.epochs = epochs
        self.step = -1
        self.init_enc_ratio = 0.8
        self.init_pub_ratio = 1 - self.init_enc_ratio
        self.ratio_step()

    def ratio_step(self):
        self.step += 1
        self.enc_ratio = self.init_enc_ratio - self.step/self.epochs * (self.init_enc_ratio - 0.5)
...
def train(loader, model, opt, criterion):
    ...
    for i, (imgs, masks) in enumerate(loader):
        ...
        opt.zero_grad()
        pred_front, pred_back = model(imgs)
        # loss = criterion(pred_front, masks) + criterion(pred_back, 1-masks)
        # loss = criterion(pred_front*masks, masks) + criterion(pred_back*(1-masks), 1-masks)
        loss = criterion(pred_front, pred_back, masks)
        iou = compute_iou(masks.data, pred_front.data)
        loss.backward()
        opt.step()
...
def main(args):
    ...
    Dual_criterion = DualLoss(ratio_decay=True, epochs=args.n_epochs).cuda()

    for i in range(1, args.n_epochs + 1):
        train_loss, train_iou = train(trainloader, unet, opt, Dual_criterion)
        valid_loss, valid_iou = valid(validloader, unet, opt, Dual_criterion)
        if Dual_criterion.ratio_decay:
            Dual_criterion.ratio_step()
